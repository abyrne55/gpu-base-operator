...
var _ = Describe("buildDeviceClasses", func() {
	r := &KMMReconciler{}
	celExpr := `device.driver == "gpu.intel.com"`

	It("uses only the base selector when ManageBinding is true", func() {
		classes := r.buildDeviceClasses(celExpr, true)
		Expect(classes).To(HaveLen(2))
		Expect(classes[1].Name).To(Equal("gpu-vfio.intel.com"))
		Expect(classes[1].Selectors).To(HaveLen(1))
		Expect(classes[1].Selectors[0].CEL.Expression).To(Equal(celExpr))
	})

	It("adds VFIO driver filter when ManageBinding is false", func() {
		classes := r.buildDeviceClasses(celExpr, false)
		Expect(classes).To(HaveLen(2))
		Expect(classes[1].Name).To(Equal("gpu-vfio.intel.com"))
		Expect(classes[1].Selectors).To(HaveLen(2))
		Expect(classes[1].Selectors[1].CEL.Expression).To(ContainSubstring("vfio-pci"))
	})

	It("does not add VFIO filter to the base gpu device class", func() {
		classes := r.buildDeviceClasses(celExpr, false)
		Expect(classes[0].Name).To(Equal("gpu.intel.com"))
		Expect(classes[0].Selectors).To(HaveLen(1))
	})
})
...
var _ = Describe("KMM DRA args generation", func() {
	It("generates correct args with all options", func() {
		...
		Expect(args).To(ContainElement("--healthcheck-port=51515"))
