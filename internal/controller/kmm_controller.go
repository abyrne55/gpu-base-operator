...
const (
	...
	healthCheckPort    = 51515 // must match KMM's hardcoded draHealthcheckPort
	gpuDeviceClass     = "gpu.intel.com"
	vfioGpuDeviceClass = "gpu-vfio.intel.com"

	vfioDriverExpression = `device.attributes["gpu.intel.com"].driver == 'vfio-pci' || device.attributes["gpu.intel.com"].driver == 'xe-vfio-pci'`
...
func (r *KMMReconciler) buildNodeSelector(cp *v1alpha.ClusterPolicy) map[string]string {
	return generateNodeSelector(cp)
}
...
func (r *KMMReconciler) setDRA(mod *kmmv1beta1.Module, cp *v1alpha.ClusterPolicy) {
	...
	mod.Spec.DRA = &kmmv1beta1.DRASpec{
		...
		DeviceClasses: r.buildDeviceClasses(celExpression, cp.Spec.DynamicResourceAllocationSpec.ManageBinding),
	}
}

func (r *KMMReconciler) buildDeviceClasses(celExpression string, manageBinding bool) []kmmv1beta1.DeviceClassSpec {
	vfioSelectors := []resourcev1.DeviceSelector{
		{CEL: &resourcev1.CELDeviceSelector{Expression: celExpression}},
	}

	// When ManageBinding is off the DRA driver refuses to rebind devices, so
	// the VFIO DeviceClass must only match devices already on a VFIO driver.
	if !manageBinding {
		vfioSelectors = append(vfioSelectors, resourcev1.DeviceSelector{
			CEL: &resourcev1.CELDeviceSelector{Expression: vfioDriverExpression},
		})
	}
