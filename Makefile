...
KIND_CLUSTER ?= intel-gpu-base-operator-test-e2e

.PHONY: setup-test-e2e
setup-test-e2e: ## Set up a Kind cluster for e2e tests if it does not exist
	@command -v $(KIND) >/dev/null 2>&1 || { \
		echo "Kind is not installed. Please install Kind manually, or run \"make kind\""; \
		exit 1; \
	}
	@command -v $(HELM) >/dev/null 2>&1 || { \
		echo "Helm is not installed. Please install Helm manually, or run \"make helm\""; \
		exit 1; \
	}
...
test-e2e: setup-test-e2e manifests generate fmt vet
	KIND_CLUSTER=$(KIND_CLUSTER) go test ./test/e2e/ -v -ginkgo.v --ginkgo.label-filter "!long" -timeout 12m
	$(MAKE) cleanup-test-e2e

.PHONY: test-e2e-extended
test-e2e-extended: setup-test-e2e manifests generate fmt vet
	KIND_CLUSTER=$(KIND_CLUSTER) go test ./test/e2e/ -v -ginkgo.v -timeout 15m
	$(MAKE) cleanup-test-e2e
...
PKG = github.com/intel/gpu-base-operator
GIT_INFO = $(shell git describe --tags --always HEAD)
BUILD_DATE = $(shell date -u +"%Y-%m-%dT%H:%M:%SZ")
BASE_IMAGE ?= gcr.io/distroless/static:nonroot

DOCKER_BUILD_ARGS = --build-arg PKGNAME=${PKG} --build-arg GITINFO=${GIT_INFO} --build-arg=BUILDDATE=${BUILD_DATE} --build-arg BASE_IMAGE=${BASE_IMAGE}

# If you wish to build the manager image targeting other platforms you can use the --platform flag.
# (i.e. docker build --platform linux/arm64). However, you must enable docker buildKit for it.
# More info: https://docs.docker.com/develop/develop-images/build_enhancements/
.PHONY: docker-build
docker-build: operator-build fwfiles-build fwupdater-build

.PHONY: operator-build
operator-build: ## Build docker image with the manager.
	$(CONTAINER_TOOL) build --pull ${DOCKER_BUILD_ARGS} -t ${IMG} -f build/operator/Dockerfile .
...
.PHONY: fwfiles-build
fwfiles-build:
	$(CONTAINER_TOOL) build --pull -t ${FWFILES_IMG} -f build/fwfiles/Dockerfile .

.PHONY: fwupdater-build
fwupdater-build:
	$(CONTAINER_TOOL) build --pull -t ${FWUPDATER_IMG} -f build/fwupdater/Dockerfile .

.PHONY: fwupdater-mock-build
fwupdater-mock-build:
	$(CONTAINER_TOOL) build --pull -t ${FWUPDATER_MOCK_IMG} -f build/fwupdater-mock/Dockerfile .
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name intel-gpu-base-operator-builder
	$(CONTAINER_TOOL) buildx use intel-gpu-base-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm intel-gpu-base-operator-builder
	rm Dockerfile.cross
...
$(HELM): $(LOCALBIN)
	...
	USE_SUDO=false HELM_INSTALL_DIR=$(LOCALBIN) $(LOCALBIN)/get_helm.sh
...
check-generated-files: crdsync
	@echo "Checking that generated files are up to date..."
	@git diff --exit-code || (echo "Generated files are not up to date." && exit 1)
...
helm-update-dependencies:
	@helm dependency update charts/gpu-base-operator

helm-package-chart: helm-update-dependencies
	chart_version=$$(yq .version charts/gpu-base-operator/Chart.yaml); \
	release_version=$$(yq .appVersion charts/gpu-base-operator/Chart.yaml); \
	rm -rf .charts; \
	echo "Packaging 'gpu-base-operator' chart with version $${chart_version} and application version $${release_version}"; \
	helm package charts/gpu-base-operator --destination .charts

helm-package-policy-chart: helm-update-dependencies
	chart_version=$$(yq .version charts/gpu-base-operator-policy/Chart.yaml); \
	release_version=$$(yq .appVersion charts/gpu-base-operator-policy/Chart.yaml); \
	rm -rf .charts; \
...
helm-push-policy-chart: helm-package-policy-chart
	helm push .charts/intel-gpu-base-operator-policy-*.tgz oci://${RELEASE_REGISTRY}
