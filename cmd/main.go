...
import (
	...
	nfdv1 "sigs.k8s.io/node-feature-discovery/api/nfd/v1alpha1"
...
const (
	defaultOperatorNamespace = "intel-gpu-base-operator"
...
func init() {
	...
	utilruntime.Must(intelcomv1alpha1.AddToScheme(scheme))
...
func detectClusterFeatures() (map[string]bool, error) {
	...
	for _, group := range apiGroups.Groups {
		if slices.Contains(openShiftGroups, group.Name) {
			features[openshiftCluster] = true
		}
...
func main() {
	...
	// Select TLS1.2 as with it, we can select cipher suites.
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "ecb1d962.intel.com",
		// Restrict the cache for namespace-scoped resources to the operator's own namespace.
		// Cluster-scoped resources (nodes, CRDs, etc.) are always cached cluster-wide.
		Cache: cache.Options{
			...
			ByObject: map[client.Object]cache.ByObject{
				// Pods are watched cluster-wide for the firmware update controller
				// which needs to list/delete Job pods in any namespace.
				&corev1.Pod{}: {Namespaces: map[string]cache.Config{cache.AllNamespaces: {}}},
				// ResourceClaims are namespace-scoped but live in workload namespaces,
				// not the operator namespace — watch cluster-wide so anyAllocatedResourceClaims
				// can detect in-use GPU claims before tearing down DRA.
				&resv1.ResourceClaim{}: {Namespaces: map[string]cache.Config{cache.AllNamespaces: {}}},
			},
		},
	...
	features, err := detectClusterFeatures()
	if err != nil {
		setupLog.Error(err, "unable to detect cluster features")
		os.Exit(1)
	}

	setupLog.Info("Detected cluster features", "features", features)
	...
	if err := (&controller.GPUFirmwareUpdateReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr, copts); err != nil {
